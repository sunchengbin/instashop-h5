...
require(['lang', 'lazyload', 'ajax', 'config', 'base', 'common', 'buyplug', 'slide', 'cart', 'fastclick', 'contact', 'viewer', 'item', 'dialog', 'debug', 'sharecoupon', 'oauth', 'cache', 'sharebargain'], function (Lang, Lazyload, Ajax, Config, Base, Common, Buyplug, Slide, Cart, Fastclick, Contact, Viewer, Item, Dialog, Debug, Sharecoupon, Oauth, Cache, Sharebargain) {
    var ITEM = {
        init: function () {
            ...
            if (init_data && init_data.code == 200) {
                ...
                try {
                    ...
                    _this.bargainInit();
                    // 根据砍价活动计算sku
                    init_data = _this.computeAndUpdateSkuPriceForBargain();
                    Buyplug({
                        data: init_data,
        ...
        computeAndUpdateSkuPriceForBargain: function () {
            var _this = this;
            var _bargain_result_price = _this.getBargainAmplitudePrice();

            // 有sku
            if (init_data.item.sku.length > 0) {
                for (var i = 0; i < init_data.item.sku.length; i++) {
                    var _curSku = init_data.item.sku[i];
                    _curSku.price = ~~_curSku.price - ~~_bargain_result_price;
                }
            } else {
                // 无sku 商品价格重置为 原价减去砍价总幅度
                init_data.item.price = ~~init_data.item.price - ~~_bargain_result_price;
            }
            return init_data;
        },
        //创建砍了多少价弹窗
        ...
        computeBargainPrice: function () {
            //自砍
            var _amplitude = (~~init_data.item.price - ~~init_data.item.bargain.base_price) * 0.3;
            return {
