 ...
 * 添加到购物车插件
 */
define(['common', 'base', 'hbs', 'text!views/moudle/distributorbuyplug.hbs', 'btn', 'dialog', 'cart', 'lang', 'config', 'fastclick', 'bargain'], function (Common, Base, Hbs, Buyplughtm, Btn, Dialog, Cart, Lang, Config, Fastclick, Bargain) {
    ...
    BuyPlug.prototype = {
        ...
        handelFn: function () {
            ...
            $(_config.wraper).on('click', _config.btn, function () {
                ...
                _config.data.has_drop_price = _config.has_drop_price;
        ...
        changeDropPrice:function(config,num){//切换分销商品价格
            var _price = config.data.item.drop_price;
            $.each(config.data.item.drop_prices,function(i,item){
                if(item.max_num != 0){
                    if(num >= item.min_num && item.max_num >= num){
                        _price = item.price;
                    }
                }else {
                    if(num >= item.min_num){
            ...
            $('.j_buy_info_price').html('Rp '+Base.others.priceFormat(_price));
        },
