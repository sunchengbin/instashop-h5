...
function getFontCss($url,$folder_name){
    if($folder_name){
        return '<style>@font-face {font-family: "iconfont";src: url("'.$url.'/css/'.$folder_name.'/base/fonts/iconfont.ttf?v=1496802263746") format("truetype"),url("'.$url.'/css/base/fonts/iconfont.svg?v=1496802263746#iconfont") format("svg");}</style>';
    }else{
        return '<style>@font-face {font-family: "iconfont";src: url("'.$url.'/css/base/fonts/iconfont.ttf?v=1496802263746") format("truetype"),url("'.$url.'/css/base/fonts/iconfont.svg?v=1496802263746#iconfont") format("svg");}</style>';
    }
}
function getIco($url){
...
function initPhpJs($js_name){
    ...
    if(isDebug()){
        return '<script src="'.STATIC_HOST.'/js/base/require-config.js"></script><script src="'.STATIC_HOST.'/js/app/'.$js_name.'.js?v=1496802263746"></script>';
    }else{
        if(FACEBOOK_JS){
            $skin_info = $skin_info.FACEBOOK_JS;
        }
        return $skin_info.'<script src="'.STATIC_HOST.'/js/dist/app/'.$js_name.'.js?v=1496802263746"></script>';
    }
}
function initPhpCss($css_name,$folder){
    ...
    if(isDebug()){
        if(TEMP_FOLDER){
            return $static_info.'<link href="'.STATIC_HOST.'/css/dist/'.$folder_name.'app/'.$css_name.'.css?v=1496802263746" rel="stylesheet"/>';
        }else{
            return $static_info.'<link href="'.STATIC_HOST.'/css/app/'.$css_name.'.css?v=1496802263746" rel="stylesheet"/>';
        }
    }else{
        return $static_info.'<link href="'.STATIC_HOST.'/css/dist/'.$folder_name.'app/'.$css_name.'.css?v=1496802263746" rel="stylesheet"/>';
    }
